...
try {
    // Captura dados da propriedade
    $titulo = filter_input(INPUT_POST, "titulo", FILTER_SANITIZE_SPECIAL_CHARS);
    $tamanho = filter_input(INPUT_POST, "tamanho", FILTER_SANITIZE_SPECIAL_CHARS);
    $tipo_solo = filter_input(INPUT_POST, "tipo_solo", FILTER_SANITIZE_SPECIAL_CHARS);
    $estado = filter_input(INPUT_POST, "estado", FILTER_SANITIZE_SPECIAL_CHARS);
    $cidade = filter_input(INPUT_POST, "cidade", FILTER_SANITIZE_SPECIAL_CHARS);
    $coordenadas = filter_input(INPUT_POST, "coordenadas", FILTER_SANITIZE_SPECIAL_CHARS);
    $id = $_SESSION["idUsuario"];  // ID do proprietário

    // Inserir Propriedade
    $sql = "INSERT INTO propriedade (titulo, tamanho, tipo_solo, estado, cidade, coordenadas, id) VALUES (?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $result = $stmt->execute([$titulo, $tamanho, $tipo_solo, $estado, $cidade, $coordenadas, $id]);

    if ($result) {
        // Verifica e processa as imagens
        $imagem1 = $imagem2 = $imagem3 = null;

        // Verificar se o arquivo foi enviado corretamente e se não houve erros
        if (isset($_FILES['imagem1']) && $_FILES['imagem1']['error'] == 0) {
            $imagem1 = fopen($_FILES['imagem1']['tmp_name'], 'rb'); // Abrir o arquivo em modo binário
        }

        if (isset($_FILES['imagem2']) && $_FILES['imagem2']['error'] == 0) {
            $imagem2 = fopen($_FILES['imagem2']['tmp_name'], 'rb'); // Abrir o arquivo em modo binário
        }

        if (isset($_FILES['imagem3']) && $_FILES['imagem3']['error'] == 0) {
            $imagem3 = fopen($_FILES['imagem3']['tmp_name'], 'rb'); // Abrir o arquivo em modo binário
        }

        // Inserir imagens associadas à propriedade
        $sqlImg = "INSERT INTO imgPropriedade (id_propri, imagem1, imagem2, imagem3) VALUES (?, ?, ?, ?)";
        $stmtImg = $conn->prepare($sqlImg);
        $id_propri = $conn->lastInsertId();  // Obtém o ID da propriedade recém-criada

        // Vincular as imagens como dados binários (LOB)
        $stmtImg->bindParam(1, $id_propri, PDO::PARAM_INT);
        $stmtImg->bindParam(2, $imagem1, PDO::PARAM_LOB);
        $stmtImg->bindParam(3, $imagem2, PDO::PARAM_LOB);
        $stmtImg->bindParam(4, $imagem3, PDO::PARAM_LOB);

        // Executar a inserção das imagens
        $stmtImg->execute();

        // Redirecionar para a página de formulário com mensagem de sucesso
        $_SESSION['result'] = true;
        header("Location: form_cadastrar_propriedade.php");
        exit();
    } else {
    ...
    // Redirecionar para a página de formulário com mensagem de erro
    header("Location: form_cadastrar_propriedade.php");
    exit();
}
?>
