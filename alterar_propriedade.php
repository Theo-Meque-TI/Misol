        ...
        <div class="content first-content">
            ...
            <div class="divao">              
                ...
                try {
                    ...
                    $sql = "UPDATE propriedade SET titulo = ?, tamanho = ?, tipo_solo = ?, estado = ?, cidade = ?, coordenadas = ? WHERE id_propri = ?";
                    $stmt = $conn->prepare($sql);
                    $result = $stmt->execute([$titulo, $tamanho, $tipo_solo, $estado, $cidade, $coordenadas, $id_propri]);
                    ...
                    if ($result) {
                        ...
                        // Atualizar as imagens associadas à propriedade
                        $sqlImg = "UPDATE imgPropriedade SET imagem1 = ?, imagem2 = ?, imagem3 = ? WHERE id_propri = ?";
                        $stmtImg = $conn->prepare($sqlImg);
                        $stmtImg->bindParam(1, $imagem1, PDO::PARAM_LOB);
                        $stmtImg->bindParam(2, $imagem2, PDO::PARAM_LOB);
                        $stmtImg->bindParam(3, $imagem3, PDO::PARAM_LOB);
                        $stmtImg->bindParam(4, $id_propri, PDO::PARAM_INT);
                        $stmtImg->execute();
                        ...
                        header("Location: form_alterar_propriedade.php?id_propri=$id_propri");
                        exit();
                    } else {
                        // Em caso de erro de atualização
                        throw new Exception("Erro ao atualizar a propriedade.");
                    }
                } catch (Exception $e) {
                    // Tratamento de erro
